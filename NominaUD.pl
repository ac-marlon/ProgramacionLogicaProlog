...
nominaInter(Total) :- findall(B, (vinculacion(A,C),horasVinculacion(C,D),categoria(A,P),valorHoraCategoria(C,P,E),liquidacionBonoMensual(C,F), B is round(((D*E*4)*(1+(F/100))))), Lista), sumar(Lista,Total).
